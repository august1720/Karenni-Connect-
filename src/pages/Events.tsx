    ...
    <div className="flex flex-col gap-6 pt-4 pb-24">
      ...
      {isCreating && (
        ...
        >
          ...
          <div className="space-y-3">
            <input type="text" placeholder="Event Title" value={newEvent.title} onChange={e => setNewEvent(f => ({ ...f, title: e.target.value }))} className="w-full px-4 py-2 bg-slate-50 dark:bg-slate-700 rounded-xl text-sm outline-none" />
            ...
            <input type="text" placeholder="Time (e.g. 2:00 PM - 4:00 PM)" value={newEvent.time} onChange={e => setNewEvent(f => ({ ...f, time: e.target.value }))} className="w-full px-4 py-2 bg-slate-50 dark:bg-slate-700 rounded-xl text-sm outline-none" />
            <input type="text" placeholder="Platform (e.g. Google Meet)" value={newEvent.type} onChange={e => setNewEvent(f => ({ ...f, type: e.target.value }))} className="w-full px-4 py-2 bg-slate-50 dark:bg-slate-700 rounded-xl text-sm outline-none" />
            <Button onClick={handleCreateEvent} className="w-full rounded-xl bg-[#D62828] text-white">Create</Button>
          </div>
        </motion.div>
      )}
      ...
      >
        ...
        <div className="z-10 flex flex-col w-full">
          <span className="px-3 py-1 bg-[#D62828]/10 text-[#D62828] dark:bg-[#D62828]/20 dark:text-[#FCA5A5] rounded-full text-[10px] font-bold uppercase tracking-wider w-max mb-4">Featured</span>
          <h2 className="text-3xl font-bold tracking-tight leading-none mb-3 text-slate-900 dark:text-white">Edu Summit <br/><span className="text-[#D62828]">2026</span></h2>
          <p className="text-slate-500 dark:text-slate-400 text-sm font-medium mb-6">Annual virtual gathering for all student networks.</p>
          
          <div className="bg-slate-50 dark:bg-slate-700/50 rounded-2xl p-4 flex justify-between items-center mb-6">
            <div className="flex flex-col">
              <span className="text-[10px] text-slate-500 uppercase font-bold tracking-wider mb-1">Date & Time</span>
          ...
          <div className="flex items-center justify-between w-full">
            ...
            <Button variant="primary" className="rounded-full px-6 bg-[#D62828] hover:bg-[#D62828]/90 text-white shadow-lg shadow-[#D62828]/20">Join</Button>
          </div>
        </div>
      </motion.div>

      {/* Upcoming List */}
      <h2 className="text-xl font-bold px-2 mt-2">Upcoming Events</h2>
      <div className="space-y-4 mx-1">
        {loading ? (
          ...
          <p className="text-center text-slate-500 text-sm mt-4">No events found.</p>
        )}
      </div>
    </div>
  );
